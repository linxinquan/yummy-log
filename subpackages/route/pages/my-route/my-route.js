...
function inferTag(name) {
 ...
 if (
  /店|馆|面|饭|咖啡|茶|酒|餐|小吃|甜品|火锅|奶茶|烧烤|糖水|包|饼|馍/.test(name)
 )
  return "美食";
 return "景点";
}
 ...
 return stripEditState(daySections).map((day, dayIndex) => {
  const rawItems = (day.items || []).map((item, itemIndex) => {
   ...
   return {
    ...
    image:
     item.image || matched?.image || matched?.logo || "/images/app-logo.jpg",
    type: itemType,
...
function buildLegacyRouteData(daySections) {
 ...
 const daySummaries = cleanSections.map((day, index) => ({
  ...
  image:
   (day.items && day.items[0] && day.items[0].image) || "/images/app-logo.jpg",
 }));

 const dayDetails = cleanSections.map((day) =>
  (day.items || []).map((item) => ({
   name: item.name,
...
function buildSummaryText(daySections) {
 ...
 const placeCount = daySections.reduce(
...
function buildPlacePickerData() {
 ...
 const allPlaces = [...placesData.getAllPlaces(), ...userAddedShops].map(item => ({
  ...item,
  // 确保有 type 字段（美食或景点）
  ...
  ids.forEach((id) => {
   ...
   if (existed) {
    allMap.set(candidate.sourceKey, {
     ...
     sourceText:
      existed.sourceType === source ? candidate.sourceText : "已在想去和收藏",
    });
   } else {
...
Page({
 ...
 applyRoute(route) {
  ...
  const daySections = syncDaySections(
 ...
 refreshMapPreview(daySections, previewIndex = 0, currentDayOverride) {
  ...
  if (currentPlace) {
   const lat =
    currentPlace.lat !== undefined ? currentPlace.lat : currentPlace.latitude;
   const lng =
    currentPlace.lng !== undefined ? currentPlace.lng : currentPlace.longitude;
 ...
 onStartNavigation() {
  ...
  const firstPlace =
   firstDayWithPlace && firstDayWithPlace.items && firstDayWithPlace.items[0];
 ...
 buildSavedRoutePayload() {
  const cleanedSections = syncDaySections(
   this.data.daySections,
   this.data.cityInfo
  );
 ...
 onConfirmReorderOption(e) {
  ...
  const optimizedSections = syncDaySections(
   currentSections,
   this.data.cityInfo
  );
 ...
 resolveDragTarget(currentY) {
  ...
  if (!targetDay) {
   targetDay = layouts.reduce((nearest, day) => {
    ...
    return Math.abs(day.center - currentY) < Math.abs(nearest.center - currentY)
     ? day
     : nearest;
   }, null);
  }
