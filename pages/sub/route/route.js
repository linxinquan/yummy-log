...
Page({
  ...
  loadRoute() {
    ...
    const likedIds = userData.likedShops || []

    const userShops = util.loadData('userAddedShops', [])
    const allShops = [...shopData.shops, ...userShops]
    const rawLikedShops = allShops.filter(s => likedIds.includes(s.id))

    if (rawLikedShops.length === 0) {
    ...
    if (selectMode === 'all') {
      const allLikedShops = rawLikedShops.map(s => ({ ...s, selected: true, orderNum: '' }))
      const routeShops = this._planAndAnnotate(rawLikedShops)
      ...
      this.setData({
        allLikedShops,
        routeShops,
        selectedCount: rawLikedShops.length,
        isAllSelected: true
      })
    } else {
      ...
      const allLikedShops = rawLikedShops.map(s => {
  ...
  _planAndAnnotate(shops) {
    ...
    const routeShops = util.planRoute(shops, startPoint)
  ...
  updateMap() {
    ...
    const points = [
      ...
      ...routeShops.map(shop => ({ latitude: shop.lat, longitude: shop.lng })),
      { latitude: startPoint.lat, longitude: startPoint.lng }  // 回到起点
    ]
    ...
    if (routeShops.length > 0) {
      ...
      centerLat = routeShops[midIdx].lat
      centerLng = routeShops[midIdx].lng
    }
  ...
  onFitRoute() {
    ...
    const lats = [startPoint.lat, ...routeShops.map(s => s.lat)]
    const lngs = [startPoint.lng, ...routeShops.map(s => s.lng)]
  ...
  _updateNavMap() {
    ...
    const polyline = [{
      points: [
        ...
        { latitude: currentShop.lat, longitude: currentShop.lng }
      ],
      color: '#00D9C0',
      width: 6,
      dottedLine: true
  ...
  onClearRoute() {
    wx.showModal({
      ...
      success: (res) => {
        if (res.confirm) {
          this.data.allLikedShops.forEach(shop => util.toggleLike(shop.id))
          this.loadRoute()
